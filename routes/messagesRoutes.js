...
<!-- End Google Tag Manager -->
    ...
    <style>
        ...
        * {
        ...
        .history-item-body > div:first-child {
...
<!-- End Google Tag Manager (noscript) -->
    ...
    <script>
        ...
        function createMessageCard(messageGroup) {
            ...
            const hasEmailStatus = messageGroup.emailStatus && messageGroup.emailStatus !== 'not_found' && messageGroup.emailStatus !== 'error';
            ...
            card.innerHTML = `
                ...
                <div class="email-section-new" data-email-status="${messageGroup.emailStatus || 'none'}">
                    ...
                    <div class="email-section-content">
                        ${hasEmailStatus && emailAddress ? createEmailVerificationDisplay(messageGroup) : createEmailFinderButton(messageGroup)}
                    </div>
                </div>
                ...
                case 'not_found':
                    return `<div class="email-content-box" data-status="not_found">
                        <div class="email-address-row">
                            ...
                            <span class="verified-check invalid"><i class="fas fa-minus-circle"></i> Not Found</span>
                        </div>
                        <div class="email-status-info">
                            <span class="email-searched-date">
                                <i class="fas fa-calendar-alt"></i> Searched: ${verifiedDate || 'Recently'}
            ...
            allMessageTypes.forEach((msgType, index) => {
                ...
                html += `
                    <div class="history-timeline-item ${isDisabled ? 'history-item-disabled' : ''}" style="margin-bottom: 0.5rem;">
                        ...
                        <div class="history-item-body" id="${uniqueId}" style="display: none; padding: 1rem; background: white; border-left: 4px solid var(--primary-purple); margin-left: 0.5rem; overflow: hidden;">
                            ${hasMessage ? `
                                <div style="font-style: normal; font-size: 0.9rem; line-height: 1.6; color: #374151; white-space: pre-wrap; margin-bottom: 1rem; text-align: left; word-wrap: break-word; overflow-wrap: break-word;">
                                    ${messageText}
                                </div>
                                ...
                                <div style="display: flex; flex-direction: column; gap: 0.75rem; padding-top: 1rem; border-top: 1px solid #E5E7EB;">
                                    <div style="display: flex; align-items: center; gap: 1rem;">
                                        ...
                                        <div class="toggle-switch ${sentStatus}" data-message-id="${messageId}" data-field="sent" onclick="toggleStatus(${messageId}, 'sent'); event.stopPropagation();" style="position: relative; display: inline-block; width: 50px; height: 26px; cursor: pointer; background-color: ${sentStatus === 'yes' ? '#10B981' : (sentStatus === 'no' ? '#EF4444' : '#D1D5DB')}; transition: 0.3s; border-radius: 26px;">
                                    ...
                                    <div style="display: flex; align-items: center; gap: 1rem;">
                                        ...
                                        <div class="toggle-switch ${replyStatus}" data-message-id="${messageId}" data-field="gotReply" onclick="toggleStatus(${messageId}, 'gotReply'); event.stopPropagation();" style="position: relative; display: inline-block; width: 50px; height: 26px; cursor: pointer; background-color: ${replyStatus === 'yes' ? '#10B981' : (replyStatus === 'no' ? '#EF4444' : '#D1D5DB')}; transition: 0.3s; border-radius: 26px;">
        ...
        function toggleStatus(messageId, field) {
            ...
            for (const group of Object.values(groupedMessages)) {
                ...
                const allMessages = [
                    ...group.messages.linkedin.map(m => ({...m, type: 'linkedin'})),
                    ...group.messages.email.map(m => ({...m, type: 'email'})),
                    ...group.messages.connection.map(m => ({...m, type: 'connection'}))
                ];
                
                foundMessage = allMessages.find(m => m.id === messageId);
                if (foundMessage) {
                    foundGroup = group;
                    break;
                }
            ...
            console.log('[TOGGLE] Found message:', foundMessage.type, 'Current status:', foundMessage[field]);
