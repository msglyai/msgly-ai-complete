...
// Root route - redirect to sign-up for marketing
app.get('/', authenticateSession, (req, res) => {
    if (req.user) {
        // User is authenticated, redirect to dashboard
        return res.redirect('/dashboard');
    }
    // Not authenticated, redirect to sign-up for marketing
    res.redirect('/sign-up');
});

// Sign-up page
app.get('/sign-up', authenticateSession, (req, res) => {
    if (req.user) {
...
app.get('/dashboard', authenticateSession, requireAuth, (req, res) => {
...
app.get('/api', (req, res) => {
    res.json({
        message: 'Msgly.AI Server - COMPLETE LinkedIn Data Extraction with Session Management',
        status: 'running',
        version: '6.1-SESSION-MANAGEMENT',
        features: {
...
const startServer = async () => {
    try {
        ...
        app.listen(PORT, '0.0.0.0', () => {
            console.log('🚀 Msgly.AI Server with Session Management Started!');
            console.log(`📍 Port: ${PORT}`);
            console.log(`🔐 Authentication: Google OAuth + JWT + Secure Cookies ✅`);
            console.log(`🏠 Route Flow:`);
            console.log(`   / → Redirect to /sign-up (marketing)`);
            console.log(`   /sign-up → If authenticated → /dashboard`);
            console.log(`   /login → If authenticated → /dashboard`);
            console.log(`   /dashboard → Requires authentication`);
            console.log(`🍪 Session Management: Automatic redirect based on auth status ✅`);
            console.log(`🛡️ Route Protection: Dashboard requires valid session ✅`);
