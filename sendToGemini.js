...
// Enhanced sendToGemini.js - Always-aggressive preprocessing (~95% reduction) + resilient OpenAI call + optional Gemini 1.5 Flash fallback
const axios = require('axios');
const https = require('https');

// Rate limiting configuration (OpenAI GPT-5-nano)
const RATE_LIMIT = {
  DELAY_BETWEEN_REQUESTS: 1000,   // 1s between requests
  MAX_RETRIES: 3,                 // Max retry attempts
  RETRY_DELAY_BASE: 3000,         // Base delay for exponential backoff (3s)
  MAX_RETRY_DELAY: 20000          // Max retry delay (20s)
};

// AGGRESSIVE limits for consistent ~95% reduction
const OPENAI_LIMITS = {
  MAX_TOKENS_INPUT: 50000,
  MAX_TOKENS_OUTPUT: 12000,       // Safer headroom for nano responses
  MAX_SIZE_KB: 3000               // Keep strict to stabilize % reduction
};
...
async function retryWithBackoff(fn, maxRetries = RATE_LIMIT.MAX_RETRIES) {
  ...
  for (let attempt = 1; attempt <= maxRetries; attempt++) {
    try {
      ...
      console.log(`[Retry] attempt ${attempt} failed, retrying in ${Math.round(finalDelay)}ms...`);
...
// AGGRESSIVE LinkedIn HTML Preprocessor - Always ~95% reduction
function preprocessHTMLForGemini(html/*, optimizationMode = 'aggressive'*/) {
  try {
    console.log(`[Preprocess] Starting aggressive HTML preprocessing (size: ${(html.length / 1024).toFixed(2)} KB)`);

    let processedHtml = html;
    const originalSize = processedHtml.length;

    // Aggressive preprocessing - remove larger sections and noise
    ...
    console.log(`[Aggressive Reduction] Original: ${(originalSize / 1024).toFixed(2)} KB | Final: ${(finalSize / 1024).toFixed(2)} KB | Reduction: ${reductionPct.toFixed(1)}% | Est. tokens: ${finalTokens}`);

    // Guardrail: warn if reduction below 90%
    if (reductionPct < 90) {
      console.warn('[Preprocess] Reduction below 90% — review cleanup rules.');
    }

    return processedHtml;

  } catch (error) {
    console.error('[Preprocess] HTML preprocessing failed:', error);
    console.log('[Preprocess] Fallback: basic processing...');
    try {
      const fallback = html
        .replace(/<script[^>]*>[\s\S]*?<\/script>/gi, '')
        .replace(/<style[^>]*>[\s\S]*?<\/style>/gi, '')
        .replace(/\s+/g, ' ')
        .trim();
      console.log(`[Preprocess] Fallback result: ${(fallback.length / 1024).toFixed(2)} KB`);
      return fallback;
    } catch (fallbackError) {
      // **Fixed the syntax error here**: ASCII-only text and balanced quotes
      console.error('[Preprocess] Even fallback failed:', fallbackError);
      return html;
    }
  }
}
...
// Send to OpenAI GPT-5-nano (Responses API) — exact structure kept
async function sendToNano({ systemPrompt, userPrompt, preprocessedHtml }) {
  const apiKey = process.env.OPENAI_API_KEY;
  console.log('[OpenAI] Sending request to GPT-5-nano Responses API...');
...
// Gemini 1.5 Flash fallback (optional, controlled by MSGLY_FALLBACK_GEMINI=true)
async function sendToGeminiFlashFallback({ systemPrompt, userPrompt, preprocessedHtml }) {
  ...
  const body = {
    ...
    generationConfig: {
      temperature: 0,
      maxOutputTokens: 6000, // stay under Gemini's practical ceiling
  ...
  // If JSON parse fails (truncation), request the JSON suffix
  try {
    JSON.parse(text);
  } catch {
...
// MAIN function to send data to OpenAI GPT-5-nano with aggressive mode always + optional Gemini fallback
async function sendToGemini(inputData) {
  try {
    ...
    console.log('=== OPENAI GPT-5-NANO WITH AGGRESSIVE OPTIMIZATION + GEMINI FALLBACK ===');
    ...
    console.log(`[Optimize] Mode: ${optimizationMode} (ALWAYS AGGRESSIVE for ${inputData.isUserProfile ? 'USER' : 'TARGET'} profile)`);

    if (inputData.html) {
      inputType = 'HTML from Chrome Extension';
      console.log(`[Input] Type: ${inputType}`);
      console.log(`[Input] Original HTML size: ${(inputData.html.length / 1024).toFixed(2)} KB`);
      ...
      preprocessedHtml = preprocessHTMLForGemini(inputData.html);

      const estimatedTokens = estimateTokenCount(preprocessedHtml);
      console.log(`[Input] Estimated tokens: ${estimatedTokens} (Max input: ${OPENAI_LIMITS.MAX_TOKENS_INPUT})`);
...

{
  ...
  "followingCompanies": ["Company Name 1", "Company Name 2"],
  "followingPeople": ["Person Name 1", "Person Name 2"],
  "activity": [
    {
...
The full HTML is provided separately in this same request. Return ONLY valid JSON. No explanations/markdown.`;
...
${JSON.stringify(jsonData, null, 2)}`;
    ...
    console.log(`[Pipeline] Processing ${inputType} with ${optimizationMode} optimization...`);
    console.log(`[Pipeline] Total prompt length: ${(systemPrompt + userPrompt).length} characters`);

    // Enforce rate limiting
    await enforceRateLimit();

    // Try nano first, then Gemini fallback if enabled + retriable
    ...
    console.log(`[Pipeline] Raw response length: ${rawResponse.length} characters`);
    ...
    try {
      ...
      console.error('[Parse] JSON parsing failed:', parseError);
      console.log('[Parse] Raw response preview:', rawResponse.substring(0, 500) + '...');
      return {
        success: false,
        status: 500,
    ...
    console.log('=== OPENAI GPT-5-NANO WITH AGGRESSIVE OPTIMIZATION + GEMINI FALLBACK COMPLETED ===');
    console.log('[Results]');
